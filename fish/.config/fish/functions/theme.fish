# Defined in /tmp/fish.oJUBVQ/set-fish-colors.fish @ line 1
function fish_theme_mine --description 'Activate my preferred fish theme colors'
  set fish_color_autosuggestion brblack
  set fish_color_cancel -r
  set fish_color_command green
  set fish_color_comment brblack
  set fish_color_cwd brblue
  set fish_color_cwd_root red
  set fish_color_end brblack
  set fish_color_error red
  set fish_color_escape brmagenta
  set fish_color_history_current -o
  set fish_color_host normal
  set fish_color_host_remote yellow
  set fish_color_match brblack -o -b brgreen
  set fish_color_normal normal
  set fish_color_operator yellow
  set fish_color_param normal
  set fish_color_quote magenta
  set fish_color_redirection brblue -i
  set fish_color_search_match brblack -b bryellow
  set fish_color_selection -b 333
  set fish_color_status red
  set fish_color_user brgreen
  set fish_color_valid_path -u
  set fish_pager_color_prefix -o normal
  set fish_pager_color_completion normal
  set fish_pager_color_description yellow
  set fish_pager_color_progress brwhite -b cyan
  set fish_pager_color_selected_prefix -o white
  set fish_pager_color_selected_completion white
  set fish_pager_color_selected_description bryellow
  set fish_pager_color_selected_background -b 333
end
